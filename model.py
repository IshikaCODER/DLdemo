...
# ==============================================================================
# GLOBAL CONSTANTS (MUST BE DECLARED BEFORE ANY FUNCTIONS THAT USE THEM)
# ==============================================================================
IMAGENET_MEAN = [0.485, 0.456, 0.406]
IMAGENET_STD = [0.229, 0.224, 0.225]
MEAN_NP = np.array(IMAGENET_MEAN, dtype=np.float32)
STD_NP = np.array(IMAGENET_STD, dtype=np.float32)
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        B, C, _, _ = x.shape
        a = self.mlp(x.mean([2, 3]))
        m = self.mlp(x.amax([2, 3]))
...
def _unnorm(t):
    ...
    # Handle batch dimensions if a single-image batch [1, C, H, W] is passed
...
class EnhancedResNet50GAT(nn.Module):
    def __init__(self, pretrained=True, freeze=False, num_classes=4):
        ...
        self.gat = DualGATClassifier(nc=num_classes)
